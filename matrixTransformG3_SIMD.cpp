...
void	matrixTransformG3_SIMD(int* matrix, int* matrixNew, int L, int Q)
{
	...
	for (int q = 0; q < Q; q++) 
    {
		for(int i = 0; i < blockLength; i += 4)
        {
            ...
            * 
            */ 
            x3 = _mm_and_si128(x0, _mm_set_epi32(0, 0, 0xffffffff, 0xffffffff));                                // x3       = 00000,00000,x0[1],x0[0]
            __m128i from_x1 = _mm_bslli_si128(_mm_and_si128(x1, _mm_set_epi32(0, 0, 0, 0xffffffff)), 8);    // from_x1  = 00000,x1[0],00000,00000
            __m128i from_x2 = _mm_bslli_si128(_mm_and_si128(x2, _mm_set_epi32(0, 0, 0, 0xffffffff)), 12);   // from_x2  = x2[0],00000,00000,00000
            ...
            */
            ...
            x4 = _mm_and_si128(x1, _mm_set_epi32(0, 0xffffffff, 0xffffffff, 0));                                // x4 = 00000,x1[2],x1[1],00000
            from_x2 = _mm_bsrli_si128(_mm_and_si128(x2, _mm_set_epi32(0, 0, 0xffffffff, 0)), 4);            // from_x2 = 00000,00000,00000,x2[1]
            __m128i from_x0 = _mm_bslli_si128(_mm_and_si128(x0, _mm_set_epi32(0, 0xffffffff, 0, 0)), 4);    // from_x0 = x0[2],00000,00000,00000
            ...
            */
            ...
            x5 = _mm_and_si128(x2, _mm_set_epi32(0xffffffff, 0xffffffff, 0, 0));                                // x5 = x2[3],x2[2],00000,00000
            from_x0 = _mm_bsrli_si128(_mm_and_si128(x0, _mm_set_epi32(0xffffffff, 0, 0, 0)), 12);           // from_x0 = 00000,00000,00000,x0[3]
            from_x1 = _mm_bsrli_si128(_mm_and_si128(x1, _mm_set_epi32(0xffffffff, 0, 0, 0)), 8);            // from_x1 = 00000,00000,x1[3],00000
